-- Store structured palm details on contracts and expose them in contracts_view.

ALTER TABLE public.contracts
  ADD COLUMN IF NOT EXISTS palm_info jsonb;

UPDATE public.contracts
SET palm_info = (regexp_match(notes, '^\[\[PALM_INFO\]\](.*?)(?:\n|$)'))[1]::jsonb,
    notes = NULLIF(split_part(notes, E'\n', 2), '')
WHERE palm_info IS NULL
  AND notes ~ '^\[\[PALM_INFO\]\]';
